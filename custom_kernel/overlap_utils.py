...
class FutureMap:
    def __init__(
        ...
        self.token_ids_buf = None

        # DEBUG: Add logging to understand the algorithm type
        import logging

        logger = logging.getLogger(__name__)
        logger.info(f"DEBUG FutureMap init: spec_algo={spec_algo}")
        if spec_algo is not None:
            logger.info(
                f"DEBUG FutureMap init: is_eagle={spec_algo.is_eagle()}, is_ngram={spec_algo.is_ngram()}, is_none={spec_algo.is_none()}"
            )

        # Initialize token_ids_buf for all algorithms except pure EAGLE algorithms
        # This ensures NGRAM, NONE, and custom algorithms have the buffer
        if self.spec_algo is None or not self.spec_algo.is_eagle():
            logger.info(
                f"DEBUG FutureMap init: Creating token_ids_buf for spec_algo={spec_algo}"
            )
            ...
            logger.info(
                f"DEBUG FutureMap init: Skipping token_ids_buf for EAGLE algorithm {spec_algo}"
            )
    ...
    def resolve_future(self, model_worker_batch: ModelWorkerBatch):
        # DEBUG: Add logging to understand the resolution path
        import logging

        logger = logging.getLogger(__name__)

        logger.info(f"DEBUG resolve_future: spec_algo={self.spec_algo}")
        if self.spec_algo is not None:
            logger.info(
                f"DEBUG resolve_future: is_eagle={self.spec_algo.is_eagle()}, token_ids_buf exists={self.token_ids_buf is not None}"
            )

        if self.spec_algo is None or self.spec_algo.is_eagle():
            logger.info("DEBUG resolve_future: Taking EAGLE path")
            # TODO(lsyin): write future indices into spec_info.future_indices
            draft_input: EagleDraftInput = model_worker_batch.spec_info
            if draft_input is None:
                # FIXME(lsyin): No future exists, only for prefill batch, not compatible with mixed mode
                logger.info("DEBUG resolve_future: draft_input is None, returning")
                return
            ...
            # Non-eagle algorithms use token_ids_buf for future resolution
